...
def _report_lifecycle(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    """Internal lifecycle gaps/cases handler, read-only."""
    ...
    # The former transport-facing lifecycle surface was always bounded. Keep the
    # handler bounded for internal tests/composers too (trade-trace-hv19).
    if limit is None:
...
def _report_strategy_health(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    ...
    try:
        with ro_db_for_args(args) as db:
            data = report_strategy_health(db.connection, raw_filter=args.get("filter"), status=status, as_of=as_of_raw, min_sample=min_sample)
        _propagate_report_meta(ctx, data)
        return data
    except ValidationError as exc:
        raise report_filter_validation_to_tool_error(exc) from exc
...
def _work_queue_common(args: dict[str, Any], ctx: ToolContext, *, surface: str) -> dict[str, Any]:
    ...
    try:
        with ro_db_for_args(args) as db:
            ...
            data = fn(db.connection, raw_filter=args.get("filter"), as_of=as_of_raw, stale_threshold_days=stale_threshold_days, kinds=kinds, limit=limit, cursor=cursor)
        _propagate_report_meta(ctx, data)
        return data
    except ValidationError as exc:
        raise report_filter_validation_to_tool_error(exc) from exc
