...
_CASCADED_EVENT_TOOLS = {
    ...
    "memory_node.invalidated",  # bucket D (diagnostic), also skipped
    "signal.emitted",  # bucket D (diagnostic), also skipped
    "outcome.recorded",  # outcome.recorded is the event_type for outcome.add; the import-ready tool is outcome.add
    "memory_node.retained",  # event-type alias; memory.retain is the tool
    "venue.created",  # alias of venue.add
    "instrument.created",
    "snapshot.added",
    "thesis.created",
    "source.added",
...
def _id_strategy_errors(rows: list[ParsedRow], max_errors: int) -> tuple[str, list[dict[str, Any]]]:
    # Skip cascaded events per trade-trace-j5b8 — they carry their own
    # ids but don't participate in the import's id-strategy enforcement
    # (they're regenerated by the parent tool's replay).
    rows = [r for r in rows if r.tool not in _CASCADED_EVENT_TOOLS]
...
def _forward_reference_errors(rows: list[ParsedRow], max_errors: int) -> list[dict[str, Any]]:
    # Skip cascaded events per trade-trace-j5b8 — they reference ids
    # whose parent rows aren't in the import bundle by design.
    rows = [r for r in rows if r.tool not in _CASCADED_EVENT_TOOLS]
    defined: set[str] = set()
...
def _tool_errors(rows: list[ParsedRow], max_errors: int) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.tool in _CASCADED_EVENT_TOOLS:
            ...
            # skipped silently with a counter; the parent tool's replay
            # regenerates them. See _validate_rows / _import_commit for
            # the actual skip + count, and
            # docs/architecture/jsonl-replay-taxonomy.md for the policy.
            continue
    ...
    with _copy_home_for_validate(_effective_home(args)) as tmp:
        for row in rows:
            if len(out.errors) >= max_errors:
                break
            if row.tool in _CASCADED_EVENT_TOOLS:
                # Bucket-B / bucket-D skip (trade-trace-j5b8). The
                # parent tool's replay regenerates these lines under
...
def _import_commit(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    ...
    if transaction_mode == "single":
        ...
        with tempfile.TemporaryDirectory(prefix="trade-trace-import-commit-") as tmp:
            ...
            for row in rows:
                if row.tool in _CASCADED_EVENT_TOOLS:
                    # validation already counted these into cascaded_skipped.
                    continue
