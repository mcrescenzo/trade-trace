...
def _paper_exposure_report(args: dict[str, Any], ctx: ToolContext) -> dict[str, Any]:
    ...
    filled = sum(float(r["filled_quantity"]) for r in records)
    notional = sum(float(r["filled_quantity"]) * float(r["average_fill_price"] or 0) + float(r["fee_amount"] or 0) for r in records)
    return {"environment": "paper", "account_label": args.get("account_label"), "as_of": args.get("as_of") or now_iso(), "mark_source": "paper_fill_records", "confidence_label": "low" if any(r["staleness_status"] != "fresh" for r in records) else "medium", "staleness_statuses": sorted({r["staleness_status"] for r in records}), "source_precedence": "paper_fill_records_only; imported/live account truth excluded", "paper_exposure": {"filled_quantity": filled, "cost_basis_plus_fees": notional}, "records": records, "paper_only": True, "non_executing": True, "no_live_execution_claims": True}


def register_paper_fill_tools(registry: ToolRegistry) -> None:
    examples = WRITE_TOOL_EXAMPLES.get("paper_fill.record", {})
    props = {"semantic_key": {"type": "string"}, "account_label": {"type": "string"}, "market_id": {"type": "string"}, "instrument_id": {"type": "string"}, "pretrade_intent_id": {"type": "string"}, "side": {"type": "string", "enum": ["buy", "sell"]}, "outcome_side": {"type": "string", "enum": ["yes", "no"]}, "requested_quantity": {"type": "number"}, "limit_price": {"type": "number"}, "book_levels": {"type": "array"}, "reference_mid_price": {"type": "number"}, "slippage_cap_bps": {"type": "number"}, "fee_amount": {"type": "number"}, "quote_id": {"type": "string"}, "book_id": {"type": "string"}, "snapshot_id": {"type": "string"}, "snapshot_as_of": {"type": "string"}, "order_as_of": {"type": "string"}, "max_snapshot_age_seconds": {"type": "integer"}, "caveats": {"type": "array"}, "evidence_json": {"type": "object"}, "provenance_json": {"type": "object"}, "idempotency_key": {"type": "string"}, "home": {"type": "string"}}
