...
- **Not a remote dashboard or trading UI.** The optional `trade-trace[console]` extra ships a **local, read-only** review dashboard at `http://127.0.0.1:8765` (see [`docs/CONSOLE.md`](./CONSOLE.md)). Outputs are JSON-by-default for agents; the Console is a read-only renderer over the same JSON, not a trading workflow.
