...
Install locally with `pip install -e .` from the repository, initialize a journal with `tt journal init`, then connect through either the MCP stdio server (`trade-trace-mcp`, configured for clients such as Claude Desktop, Cursor, Windsurf, and Cline) or the JSON-first CLI (`tt` / `trade-trace`). MCP tool names use dot notation, and the CLI maps dots to spaces; both transports share the same envelope shape and error semantics described in [docs/architecture/contracts.md](./architecture/contracts.md). Use an `actor_id` such as `agent:research-bot` and never submit broker/API credentials or secrets.
...
1. `instrument.add` — create or identify the market/instrument under a venue. Keep `resolution_criteria_text` explicit enough that future outcome resolution is auditable.
...
2. `thesis.add` — record why the trade/skip is being considered, including falsification criteria and optional `strategy_id`.
...
```json
{"tool":"forecast.add","args":{"thesis_id":"ths_...","kind":"binary","outcomes":[{"label":"YES","probability":0.58},{"label":"NO","probability":0.42}],"idempotency_key":"agent-run-42:forecast:event-x:v1"}}
```

4. `decision.add` — record the actual action (`buy`, `sell`, `hold`, `skip`, etc. per schema), rationale, tags, and optional strategy linkage.

```json
{"tool":"decision.add","args":{"instrument_id":"ins_...","thesis_id":"ths_...","type":"skip","reason":"Edge is smaller than spread and resolution risk.","strategy_id":"str_...","tags":["spread-discipline"],"idempotency_key":"agent-run-42:decision:event-x"}}
```

5. `outcome.add` — resolve the instrument when the result is known. This enables scoring and later review.
...
6. `memory.recall` — before writing the next thesis, retrieve relevant reflections, observations, and playbook rules by instrument or strategy context.
...
8. `playbook.propose_version` — when a reflection should change future procedure, propose a new playbook version anchored to the reflection node.
...
- Tool arguments vs transport metadata: pass `actor_id` through the transport/core metadata where your client supports it; do not assume putting it inside every `args` object changes the envelope actor.
