...
| AX-028 | 2026-06-04-03 | report | `report.open_positions`/`report.current_exposure` left an open row's stale **non-null** `unrealized_pnl` in place while attaching a fresher `latest_mark` — intra-row contradiction (live: ETH row `latest_mark.price=0.415` but `unrealized_pnl=+$1.00`, marked against the prior 0.49 snapshot). The AX-025 fix only re-marked the null-PnL case. Fix: shared `_position_row_payload` now always re-marks side-aware from the latest mark's YES-contract price. Distinct from AX-025 (null case) and AX-027 (which *price*); this is *staleness of a non-null stored mark*. `report.pnl` remains under pr2j. | fixed | 3824370 (+regression test; reverted-fix confirms repro) |
