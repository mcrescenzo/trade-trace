...
The default public registry exposes the consolidated v0.0.2 catalog. Renamed tools keep the old names as hidden dispatch aliases with `renamed_to`/`redirect` metadata (for example `outcome.add` redirects to `resolution.add`; `playbook.create` and `strategy.create` redirect to `playbook.upsert` and `strategy.upsert`). `playbook.propose_version` stays catalog-visible — it is the only tool that mints a `playbook_version_id`, and `playbook.record_adherence` (which feeds `report.playbook_adherence`) hard-requires one, so folding the producer would leave the adherence report structurally unreachable from the MCP catalog (trade-trace-47tp). `tool.schema`/MCP metadata is the authoritative way for legacy callers to discover these hints and removed-tool guidance. For the public forecast/decision setup path, clients should call `market.bind` first and use its returned `instrument_id` for `snapshot.add`/`decision.add`; `forecast.add` can create the folded thesis prerequisite when supplied that `market_id` or `instrument_id` plus `rationale_body`.
