...
**Implementation mapping (trade-trace-bino).** The two components above land in different places in `tools/memory.py`. The `confidence_base * exp(-decay_rate_per_day * age_days)` decay factor is computed by `_effective_confidence` and is what the `min_confidence` recall filter compares against (`_shape_recall_items`): a high-decay node falls below the caller's `min_confidence` threshold once enough wall-clock time has elapsed, while a null/zero `decay_rate_per_day` is a no-op and the node filters on its stored `confidence_base` exactly. `age_days` is the fractional-day gap between `created_at` and the recall's effective timestamp (`as_of` when set, else now); recalls earlier than `created_at` clamp to age 0. The `supersession_discount` (0.25×) is applied multiplicatively to the RRF *ranking* score in `_score_ranked_nodes` rather than folded into the confidence gate, so superseded nodes sink in top-k ordering without being silently filtered out of an explicit `min_confidence` query. `effective_confidence` is clamped to `[0, 1]`.
...
The "off by default" choice preserves the absolute air-gap promise in [`product-scope-v002.md`](product-scope-v002.md) §Safety posture on first `journal.init`. MVP recall via BM25 + temporal (+ graph if requested) returns valid results without vectors; SEMANTIC is a ranking-quality improvement, not a correctness gate.
...
- **`GRAPH`** — 1-hop BFS from the `context` node, following edges optionally filtered by edge type. Deferred from MVP-default because graph relevance only earns its keep once edge populations are dense. Available behind a strategy flag (`strategies: ["graph", ...]`) from day one for callers who want it. Always available regardless of embedding configuration; does not require network.
  - **No-context abstention (trade-trace-2iug).** The graph strategy needs a `context: {kind, id}` entrypoint to measure connectivity against. When the recall supplies no `kind` or no `id`, there is no entrypoint, so the strategy *abstains*: it returns an empty ranking rather than an id-ordered (alphabetical) sort of the whole corpus, which would be a zero-signal contribution that still forces RRF to iterate every node. Abstention is invisible to fusion — RRF (§7.4) iterates only the ranks each strategy actually supplies — so BM25/temporal/semantic remain the active signals. The strategy still appears in `strategies_used` (it was requested), but contributes no `graph` provenance to any result row.
...
- **`memory.reflect(target, body, *, importance?, derived_from?, supports?, contradicts?, supersedes?, ...)`** — sugar over `retain(node_type=reflection, ...)` that auto-wires the required `about` edge to `target`. The live schema accepts the §10 edge-sugar fields `derived_from` / `supports` / `contradicts` / `supersedes` (each a memory-node id or list of ids), writing those edges from the reflection in the same transaction; use `memory.link` for edges that do not originate at this reflection.
...
The following edge sugar is **implemented** (bead trade-trace-qikt).
Each field takes a memory-node id or a list of memory-node ids and
writes the named edge type FROM the new reflection node TO each id, in
the same transaction as the reflection node and `about` edge:

3. `derived_from` — edges to the observations/nodes this reflection was synthesized from.
4. `supersedes` — edges to older reflection (or other memory-node) ids this one replaces.
5. `supports` / `contradicts` — edges to other memory nodes this reflection bears on.

The sugar edges are co-idempotent with the reflection node and `about`
edge (bead trade-trace-1up / -5udu): a same-key reflect retry reuses any
existing identical edge instead of inserting a duplicate. Each named
target memory-node must already exist (a missing target returns
`NOT_FOUND` and rolls back the whole write). The written sugar edges are
echoed back on the response under `edges` as
`{edge_type, target_id, edge_id}` entries.

`memory.link` remains the interface for edges that do **not** originate
at a reflection node (e.g. consumer-row → memory edges per §9.1) and for
edge types other than these four.
...
- **Embeddings flipped to off-by-default in MVP.** Earlier v2 made `bge-small-en-v1.5` default-on via lazy download on first init; that violated the "no outbound calls on `journal.init`" promise in [`product-scope-v002.md`](product-scope-v002.md) §Safety posture. v3 ships vectors off; opt-in via explicit config or `tt model import`. Deps still ship in the wheel.
