...
- **Not a benchmark.** Trade Trace exports data in ForecastBench-compatible format, but it is the tooling, not the leaderboard.

## Why now

Three shifts make 2026 the right moment:

- **LLMs are crossing into competent forecasting territory.** Halawi et al. (2024) reached 71.5% accuracy on hard forecasting questions with RMS calibration error of 0.42 — within striking distance of human superforecasters (0.38). ForecastBench (2025) shows steady progression: best-performing models at Brier 0.101 vs superforecasters at 0.081. At current rates, LLMs match superforecasters within 2026. Calibration matters more than ever.
...
4. **Market-agnostic core, domain-specific adapters.** Prediction markets, equities, options, futures, crypto, and event markets share a generic decision/position/outcome spine. Venue-specific details live in connector plugins and a flexible metadata column.
5. **Local-first by default.** Storage is a single SQLite file, optionally accompanied by a JSONL audit log. No remote services required. External sync is opt-in and explicit.
6. **MCP-first, CLI-equivalent, JSON-only contract.** Every operation is exposed as an MCP tool and as an equivalent CLI command. Output is JSON to stdout; prose is suppressed by default and only emitted to stderr when `--human` is requested. Errors carry stable codes.
...
10. **The agent is the one with judgment.** The system surfaces objective signals (calibration drift, mistake-tag frequency, playbook violations) but does not decide what counts as a mistake. The agent reflects; the system stores and links the reflection.
...
Layers 1 and 2 are deterministic and live in the system. Layers 3 and 4 are agent-driven and stored as graph memory. The loop closes because every new decision is checked against the current playbook version, every override is recorded, and override outcomes feed the next cycle's reports.

This loop is the product. Everything else — connectors, exports, calibration metrics, position tracking — exists to feed it or measure it.
...
| **Polymarket Agents framework** | Polymarket-specific agent SDK | Prediction-market venue access | Multi-venue; venue connectors are plugins, not the product |
...
| **ForecastBench** | Forecasting benchmark | Calibration scoring | Tooling, not benchmark — but exports in compatible format |
...
- Polymarket-only product semantics — prediction-market fields are connector metadata and forecast extensions, not the core model.

## Safety posture

- The MVP cannot execute trades. There is no surface that signs, routes, or transmits an order.
- The MVP never reads, stores, or asks for private keys, seed phrases, broker credentials, or wallet signatures. Connectors that need authentication for read-only data may accept API keys via environment variables, but they are forbidden from write or trade operations.
...
4. The system has graded forecasts across at least two distinct asset classes (e.g., binary prediction markets + equity directional bets) using consistent calibration math.
